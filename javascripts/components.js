...
angular.module('adistalApp').constant('decoderScripts', {
    ...
    controller: function($log, $http, config, utilities, practitionerService) {
        ...
        self.assignToOrganization = function() {
            ...
            $log.info('Assign To', self.assignTo);
        };
    }
    ...
    templateUrl: '/templates/ui-components/practitioner-organizations.html',
    controller: function($log, $http, config, utilities, practitionerService, userData, $scope) {
        var self = this;
        self.specialitiesText = [];
        self.specialities = [];

        self.$onChanges = function() {
            $log.info('organizations: ', self.organizations);
        ...
        function loadSpecialities() {
            practitionerService.getSpeciality().then(function(respSpeciality) {
                if (respSpeciality.data.length > 0) {
                    self.specialities = respSpeciality.data.map(function(item) {
                        return item.value;
                    });
                    self.specialitiesText = respSpeciality.data.map(function(item) {
                        return item.value.text;
                    });

                    $log.info('loadSpecialities: ', self.specialities);
                }
            });
        }
        loadSpecialities();
        ...
        self.addOrganization = function() {
            if (self.selectOrganization && self.selectSpeciality.text) {
                ...
                var newPractitionerRole = {
                    ...
                    practitioner: {
                        reference: $scope.$parent._id,
                        display: 'practitioner'
                    }
                };
                if (self.specialitiesText.indexOf(self.selectSpeciality.text) === -1) {
                    practitionerService.saveSpeciality('post', dataSpeciality).then(function(resp) {
                        self.specialities.push(resp.data.value);
                        self.specialitiesText.push(resp.data.value.text);
                        if (indexOrganization !== -1) {
                            newPractitionerRole.specialty = [resp.data.value];
                    ...
                    if (indexOrganization !== -1) {
                        newPractitionerRole.specialty = [dataSpeciality.value];
                        self.practitionerRole.push(newPractitionerRole);
                        self.selectSpeciality = {
                            coding: [{}]
                        };
    ...
    controller: function(config, dwv, $log, $q, decoderScripts) {
        var self = this;
        var layer = null;
        self.enabled = false;
        self.app = null;
        ...
        this.$onChanges = function(changes) {
            ...
            if (titleFile.indexOf('.dcm') !== -1 || titleFile.indexOf('.png') !== -1) {
        ...
        function setControlEvent() {
            ...
            self.app.addEventListener('slice-change', function() {
    ...
    controller: function($state, $translate) {
        var self = this;
        self.$onChanges = function() {
            console.log(self.type);
            self.isAdmin = (this.type === 'Admin' || this.type === 'Node Administrator');
        };

        self.isCreator = function(item) {
            if (typeof item.orderer !== 'undefined') {
                return item.orderer.reference === self.currentUser;
            }
            return false;
        };
        ...
        if (self.properties && self.properties.length > 0) {
            self.propertyName = self.camelize(self.properties[0]);
            self.reverse = false;
        }
        ...
        self.orderRedirect = function viewRedirect(itemId) {
            $state.go(self.orderState, {
                patient_id: itemId
            });
        };
        self.reportRedirect = function viewRedirect(itemId) {
